...
| Icons | `lucide-react` (standardized this session; replaced emoji) |
...
- Everything committed on `claude/casetrack-inventory-pwa-9ngunk`; working tree clean.

### Not yet done — the actual current blockers
1. **Migrations have not all been run against the live database.** Run order: everything through `041` for the catalog, then `032` → `042` → `043` for Notes. The Notes tab will error until 042/043 are applied.
...
> **Status update:** Supabase MCP tooling has since become available in the current session, which may lift this restriction going forward. Worth re-testing.
...
**Immediate**
- [ ] Run migrations `→041`, then `032 → 042 → 043` in Supabase.
