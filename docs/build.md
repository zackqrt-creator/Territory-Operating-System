...
- **A phone stayed pinned to an old build across four deploys** (reported 2026-07-31: the sign-in screen still offered only a magic link, days after password sign-in shipped and deployed READY). The cause was the service worker's default navigation handling: `vite-plugin-pwa` precaches `index.html` and registers a `NavigationRoute` bound to that precached copy, so **every navigation was answered out of the cache**. The cached shell named old hashed bundles, and the page never asked the network whether a newer build existed — `registerType: "autoUpdate"` does not help, because it only `skipWaiting()`s the worker. Fixed by removing `html` from `globPatterns`, setting `navigateFallback: null` so no `NavigationRoute` is registered, and handling navigations with a `NetworkFirst` route (`territory-os-shell`, 5s network timeout) — online always wins, offline falls back to the last good copy. `pwa.ts` warms that cache on first load so an offline cold start still works.
- The sign-in screen now prints the build stamp and offers a **hard reset** (`hardReset()` in `pwa.ts`: unregister every worker, delete every cache, reload on a cache-busting URL). It deliberately leaves `localStorage` alone — the Supabase session lives there, and clearing it would sign someone out to fix a rendering problem. It lives on that screen because being stuck and being signed out tend to arrive together.
- The app called itself **CaseTrack** on the two screens a person actually looks at while the repo, docs and the user all called it **Territory OS**. Renamed across the login screen, Home header, `index.html` title, the PWA manifest and the README, with a real mark (`BrandMark`) replacing the "CT" initials tile.
