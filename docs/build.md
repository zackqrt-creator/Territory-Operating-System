...
2. **Notes / second brain** — a two-layer team-memory system (capture → review → durable knowledge). *(built, needs migrations run + live test)*
...
All migrations are numbered files in `supabase/migrations/`. **They must be pasted + run manually in the Supabase SQL editor** — the sandbox Claude runs in cannot reach Supabase over the network (see Blockers).
...
**Run order for the notes system:** `032` → `042` → `043` (032 provides `pg_trgm`). 033 is a prerequisite of 042 (already run).
...
**Active blocker — Supabase unreachable from the build sandbox.**
The environment Claude runs in only allows outbound HTTPS on port 443 through a policy proxy that **blocks Supabase's domains** (DB pooler *and* `api.supabase.com` both return 403). Direct Postgres (5432/6543) is also blocked and the direct host is IPv6-only. Net effect:
- Claude **cannot run migrations** or query the DB. Every migration is delivered as copy-paste SQL for the user to run manually.
- Claude **cannot click-test** against live data. All UI is verified by `tsc` typecheck + `vite build` + dev-server transform only.
- This is an infra/network-policy limit, **not** a Supabase setting — resetting the DB password / making a personal access token did not help. A different Claude Code environment network policy would be required to change it.
...
- [ ] **Run migrations in Supabase** — everything through **041** for the catalog, then **032 → 042 → 043** for Notes. The Notes tab and anything touching `territory_notes` will error until 042/043 are in.
- [ ] **Live click-through** of the new UI once logged in + migrations run: Home command center, Notes capture → review → promote-to-page, Inventory tabs (esp. Catalog/Sets showing the loaded myOPS data), the More nav sheet, `/wiki`→`/pages` redirects.
